...
open class MaterialShowcase: UIView {
  ...
  @objc public var backgroundViewType: BackgroundTypeStyle = .square
  @objc public var backgroundRadius: CGFloat = -1.0 // If the value is negative, calculate the radius automatically
  @objc public var backgroundRatio: CGFloat = 0.4
  // Tap zone settings
...
extension MaterialShowcase {
  ...
  private func addBackground() {
    ...
    case .square:
      let height: CGFloat = UIScreen.main.bounds.height * min(backgroundRatio, 1.0)
      let y = min(targetRippleView.center.y - targetRippleView.bounds.size.height/2.0 - 20, UIScreen.main.bounds.height - height)
      backgroundView = UIView(frame: CGRect(x: 0, y: y, width: UIScreen.main.bounds.width, height: height))
    }
  ...
  private func addBackgroundMask(with radius: CGFloat, in view: UIView) {
    let center = backgroundViewType != .full ? view.bounds.center : targetRippleView.center
